...
async def _get_events(store: SectorStore, sector_name: str | None = None) -> list:
    if sector_name:
        cur = await store._db.execute(
            ...
            "trading_days_since_last_sector_pick, total_sector_pick_count "
...
async def test_first_pick_no_repick(store: SectorStore):
    ...
    event_id = await store._record_sector_pick_event("반도체", _ts("2026-04-21"))
...
async def test_repick_friday_to_monday(store: SectorStore):
    ...
    id1 = await store._record_sector_pick_event("반도체", _ts("2026-04-24"))
    id2 = await store._record_sector_pick_event("반도체", _ts("2026-04-27"))
    rows = await _get_events(store, "반도체")
    assert len(rows) == 2
    r2 = rows[1]
...
async def test_repick_one_full_week(store: SectorStore):
    ...
    id1 = await store._record_sector_pick_event("반도체", _ts("2026-04-21"))
    id2 = await store._record_sector_pick_event("반도체", _ts("2026-04-28"))
    rows = await _get_events(store, "반도체")
    r2 = rows[1]
    assert r2[3] == 1                # is_sector_repick
    assert r2[4] == id1              # prev_event_id
...
async def test_repick_same_day(store: SectorStore):
    ...
    id1 = await store._record_sector_pick_event("반도체", _ts("2026-04-21"))
    id2 = await store._record_sector_pick_event("반도체", _ts("2026-04-21"))
    rows = await _get_events(store, "반도체")
    r2 = rows[1]
    assert r2[3] == 1                # is_sector_repick
    assert r2[4] == id1              # prev_event_id
...
async def test_cross_sector_isolation(store: SectorStore):
    ...
    id_semi_1 = await store._record_sector_pick_event("반도체", _ts("2026-04-21"))
    _id_ai = await store._record_sector_pick_event("AI",   _ts("2026-04-22"))
    id_semi_2 = await store._record_sector_pick_event("반도체", _ts("2026-04-28"))
...
async def test_total_count_increments(store: SectorStore):
    ...
    await store._record_sector_pick_event("반도체", _ts("2026-04-21"))
    await store._record_sector_pick_event("반도체", _ts("2026-04-22"))
    await store._record_sector_pick_event("반도체", _ts("2026-04-23"))
    rows = await _get_events(store, "반도체")
    assert len(rows) == 3
    assert rows[0][7] == 1
    assert rows[1][7] == 2
