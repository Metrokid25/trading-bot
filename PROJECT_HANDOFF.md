...
> 마지막 업데이트: 2026-08-08 (P0 paper 데이터 무결성 패치) · 작업 기준 커밋: 1356c7d

## 최근 변경 (2026-08-08) — Paper 데이터 완결성·원장 불변식 P0

- 운영 DB 진단 결과를 따라 `feature/p0-paper-data-integrity`에서 측정 신뢰성부터
  보강했다. 전략의 진입·청산 임계값은 변경하지 않았다.
- 활성 종목별 정규장 09:00~15:30 분봉 수, 유효 OHLC, 첫/마지막 시각, 내부
  최대 간격과 NXT 프리장 수집 품질을 `paper_data_quality`에 감사 기록한다.
  정규장은 98% 이상(최소 384/391분)·09:02 이전 시작·15:28 이후 종료·최대
  간격 5분을 모두 요구한다. 시점별 NXT 기대 종목은 프리장도 최소 54/59분·
  08:02 이전 시작·08:50 이후 종료·최대 간격 5분을 요구한다. 출처가 확인되지
  않은 partial/no_data가 하나라도 있으면 `finalized=1`을 금지한다.
- 프리장 0봉만으로 비-NXT를 추론하지 않는다. 넥스트레이드 공식 2026년 3분기
  전체 610종목 목록(SHA-256 고정)을 effective-dated registry로 두고, 목록 포함은
  True·완전목록의 여집합은 False로 `paper_premarket_eligibility`에 날짜별 감사
  저장한다. 시세 0/빈 응답은 eligibility 근거로 사용하지 않는다. 분기 유효기간이
  끝나면 fail-fast하며 새 공식 목록을 검증해 갱신하기 전에는 paper 기록을 막는다.
  과거 불완전일은 30/60/120분 백오프·3회 격리로 재수집하며, 목표 날짜 봉만
  원자 교체해 중간 누락·0원 OHLC와 다음날 타일 혼입을 함께 방지한다.
- 2026-07-17처럼 KRX 달력상 거래일이지만 전 종목 0봉인 실질 휴장은 운영 당시
  직접 프로브로 확인된 날짜만 명시적 override로 둔다. 해당 날짜는
  `paper_session_status=confirmed_empty`로 감사 기록하고 소급 날짜와 12일 캐시
  복구에서 제외한다. 동일 Toss 소스의 빈 응답 횟수만으로 새 휴장을 자동 확정하지
  않으므로, 다른 정상 거래일의 전 종목 0봉은 계속 미완으로 차단된다.
- 라이브 유니버스는 종목코드로 한 번만 선택하고 최신 active pick의 섹터를 쓴다.
  주도섹터와 공유현금 섹터 상한은 `sector_key`로 대소문자·공백을 통합한다.
- 커밋 직전에 현재일까지 전체 이력의 `equity[t] = equity[t-1] * (1+day_ret[t])`,
  일별 거래수=비-EOR 체결원장 수, 포트폴리오 거래수=배분원장 수를 검사한다.
  불일치는 전체 전략 기록 트랜잭션을 롤백하며, 미완/미검증 과거일을 고치지 못하면
  이후 날짜 기록을 중단한다.
- 신규 회귀 테스트는 7/14 장애 형태(09:51 종료 52/391분), 과거 partial 복구,
  코드/섹터 중복, DB 마이그레이션, 원장 불변식, 소급 중단을 포함한다.
- `scripts/preflight_paper_rebuild.py`는 운영 DB를 쓰지 않고 historical registry
  범위와 전 code-day 분봉 품질을 점검한다. 원본 스냅샷은 7/14 한 날짜에서 BLOCK,
  별도 로컬 복구본은 `PRECHECK PASS: days=25,codes=77`을 확인했다.
- 독립 리뷰에서 나온 무봉 허용·세션 시작/연속성·재수집 폭주·손상봉 미교정·
  NXT 0봉 오판·다음날 타일 혼입·registry 만료 문제를 모두 보완했고 최종
  재리뷰는 **no findings**다. 전체 `tests\` **517 passed**, 기존
  `pandas_market_calendars` warning 1건, `git diff --check` 정상이다.
- 오너 승인으로 최종 검증본을 `origin/main`에 반영한다. 미니PC 파일/DB/프로세스,
  예약 작업, 주문은 건드리지 않았고 배포도 포함하지 않는다. 기존 `paper.db`는 이미 원장 불일치가 있어
  배포와 동시에 자동 재구축하지 않는다. 백업→불완전 분봉 재수집→첫날부터 paper
  체인 재구축은 별도 오너 승인과 배포 절차가 필요하다.
...
- Fixture E2E와 최신 main 통합 후 전체 486 passed, 1 skipped. `main.py`, 주문 메서드, 실전 실행 경로는
...
## 2026-07-26 미니PC SSH 브리지 복구 및 읽기 전용 실측

- 메인 PC에서 미니PC SSH 키 등록을 재확인했다. 실제 성공 계정은
  `미니PC@100.100.141.24`이며 `hostname` 출력은 `DESKTOP-NFN1RCA`다.
  `jason@100.100.141.24`는 같은 키로 `Permission denied`가 난다.
- 사용 키는 메인 PC `C:\Users\JASON\.ssh\aws_vps`이며 공개키 fingerprint는
  `SHA256:cpbH+EMmEO6QACTjYbjuKoAAwfL9pbXJ2UzyPDjLW0k`다. 미니PC의 관리자 계정
  인증은 `C:\ProgramData\ssh\administrators_authorized_keys`에 등록되어야 한다.
- 브리지 호출 시 현재 기본 설정의 `sshUser`가 `jason`이면 실패하므로, 다음처럼
  명시한다.
...
- 단, 2026-07-26 실측에서는 브리지 스크립트가 미니PC `codex` CLI에
  `--ask-for-approval never exec ...` 인자를 한 덩어리로 전달해 실패했다.
  원인은 브리지 인자 전달 방식과 원격 CLI 파싱 불일치로 보이며, 상태 확인은
  동일 SSH 계정으로 직접 `powershell -EncodedCommand`를 보내 읽기 전용으로 수행했다.
- 미니PC `C:\trading-bot` 읽기 전용 실측 결과:
  - `main` HEAD `540949a fix: make paper metrics membership aware`
  - origin 대비 ahead/behind `0/0`
  - 미추적 `AGENTS.md` 존재 및 보존 필요
  - 상주 3프로세스 생존:
    `main_tracker.py`,
    `python -m strategy.paper_runner --market-schedule`,
    `python -m uvicorn webapp.server:app --host 0.0.0.0 --port 8000`
  - `.venv\Scripts\python.exe -m strategy.paper_runner --report` exit code 0
- `--report` 최신 누적(2026-07-24): v2 `+35.70%`, 벤치 `-18.19%`, 초과
  `+53.90%p`; gm_v3 `-5.68%`, 초과 `+12.51%p`; v4r `-54.26%`, 초과
  `-36.07%p`. 콘솔 인코딩 문제로 한글 요약 일부는 깨졌지만 표와 exit code는 정상이다.
- 이번 작업은 SSH/상태 확인만 수행했다. 파일, DB, git, 서비스, 프로세스, 주문 경로는
  변경하지 않았다.

---

## 2026-07-27 텔레그램 페이퍼 요약 개편 + top-down/v2_qv 연구

### 텔레그램 요약

- 당일 매매 12건 상한과 `… 외 N건` 생략을 제거했다. 텔레그램 4,096자 제한을
  넘으면 줄 단위로 여러 메시지에 나눠 모든 거래를 보낸다.
- `v2` 가동 여부와 당일 매매/관망 상태를 명시한다. 누적 성적은 모바일에서 열이
  유지되는 HTML `<pre>` 고정폭 표(`모드/건수/승률/평균/누적`)로 바꿨다.
- 전략명은 `BASE/+R13/+R14/+R13R14`로 줄이고 시장차와 보유종목을 표 아래에
  분리했다. 종목명은 HTML escape한다.
- `v2_portfolio`, corrected 축, 신규 `v2_qv` 축은 검증 전 텔레그램에서 숨긴다.

### 스승님 그림해설판 기반 top-down 패치

- 전일까지의 등록 유니버스 5일 수익률·MA20 breadth·상승 MA20 비율, 업종
  상대강도, 종목 MA20로 `strategy/topdown.py` 진입 허가 계층을 구현했다.
  당일 v2/v4r은 전일까지만, 다음날 시가 체결 GM은 신호일 종가까지 사용해
  미래참조를 막았다.
- 현재 웹앱 활성 76종목, 토스 실데이터 2026-01-02~07-24, 왕복비용 0.5%:
  - `gm_v3` 원본 207건 평균 +0.39%, 직렬복리 +112.8%, MDD -18.1%;
    게이트본 53건 +0.30%, +15.8%, MDD -7.1%.
  - `R13` 원본 457건 +0.31%, +253.3%, MDD -35.3%;
    게이트본 81건 +0.30%, +25.2%, MDD -9.1%.
  - `v4r` 원본 366건 +0.02%, -49.1%, MDD -78.8%;
    게이트+재진입1회 본 44건 +1.22%, +52.6%, MDD -27.2%.
- 7/6~7/24에는 게이트가 GM 7건(-5.7%), R13 19건(-12.9%), v4r 54건
  (-54.5%)을 전부 차단했다. 반대로 v2 29건 평균 +1.18%도 전부 차단했으므로
  v2에는 시장 게이트를 적용하지 않는다.
- R2 `5>20>60` 강제는 전체 평균 -0.16%로 악화되어 폐기. GM/R13의 게이트본과
  v4r 패치는 연구 코드일 뿐 현재 paper 축에는 아직 연결하지 않았다.

### v2 구조품질 연구와 최종 v2_qv

- 최초 복합점수(거래량, 높은 저점, 긴 다지기, 업종 상대강도, 과열 감점)는
  점수와 수익이 단조 관계를 보이지 않아 폐기했다.
- 개별 항목 실측에서 눌림 거래량 마름은 평균 차이 +1.07%p, 돌파봉 거래량
  증가는 +0.22%p였다. 높은 저점, 긴 다지기, 업종 상대강도, 과열 감점은 v2에서
  역효과라 제외했다.
- `v2_qv` 고정 조건:
  1. 다지기 평균 거래량 `<=` 하락구간 평균의 0.80배.
  2. 돌파봉 거래량 `>=` 다지기 평균의 1.50배.
- 직렬복리 참고치: 109건, 승률 45.0%, 왕복비용 0.5% 후 평균 +0.47%,
  누적 +50.5%, MDD -20.9%. 1~3월 +2.3%, 4~7월 +47.1%, 최근 7/6~24 +10.3%.
  3월만 월평균 -0.08%이고 나머지 6개월은 플러스였다.
- 비용 민감도: 왕복 0.5% 평균 +0.47%/누적 +50.5%, 0.8% +0.17%/+8.6%,
  1.0% -0.03%/-12.7%. 비용 1%에서는 엣지가 사라진다.
- 기존 paper와 동일한 공유현금(종목당 20%, 동시 5, 업종당 2, 청산 후 슬롯
  재사용)으로 재계산:

| 축 | 후보/선택 | 실제 NAV | MDD |
|---|---:|---:|---:|
| v2 원본 | 210 / 144 | +0.28% | -7.15% |
| **v2_qv** | 120 / 104 | **+5.51%** | **-4.57%** |

- 따라서 `+50.5%`는 실계좌 수익이 아니며, 현재 현실적 참고치는 약 6.5개월
  `+5.51%`다. 현재 등록 76종목을 과거에도 고정한 회고 테스트라 survivorship
  한계가 남는다.
- 기존 v2를 대체하지 않고 `v2_qv`(직렬 비교)와 `v2_qv_portfolio`(공유현금)를
  별도 forward 관찰축으로 paper runner에 연결했다. 배포 후 8~12주 동결하고
  실제 편입경계·슬리피지·공유현금 NAV로 판정한다.

### 검증·배포 상태

- 정식 `tests\` 전체 **447 passed**, 기존
  `pandas_market_calendars` warning 1건. 저장소 루트 `pytest`는 독립
  `test_connection.py`/`test_telegram.py`의 미설치 `requests` 때문에 수집 실패하므로
  계속 `pytest tests\ -q`를 사용한다.
- 로컬 개발·백테스트만 수행했다. 미니PC 파일/DB/서비스/프로세스/주문은 변경하지
  않았다. 배포 시 ff-only pull 후 tests 전체 실행, 장 마감 후 paper runner만
  WMI 재기동하고 `v2_qv` 두 축 생성·기존 축 불변·텔레그램 비노출을 확인한다.

---

## 2026-08-02 노트북 인수인계 + GM Regime v1 연구 보존

- 시작 실측: 로컬 `main`과 `origin/main`은 `3f5cf21`에서 ahead/behind `0/0`.
  기존 미추적 `tmp/` PDF 렌더·텍스트 추출 산출물은 삭제·이동·추적하지 않았다.
- 2026-07-27 이후 로컬에 남아 있던 연구 파일 4개를 검증해 인수인계에 포함했다:
  `strategy/gm_regime_v1.py`, `backtest/run_gm_regime_v1.py`,
  `tests/test_gm_regime_v1.py`, `docs/research/gm_regime_v1.md`.
- `gm_regime_v1`은 기존 `v2_qv` 공유현금 시그널을 삭제하지 않고
  `TopDownGate` 장세에 따라 당일 노출만 risk-on 100%, neutral 50%, risk-off
  20%로 줄이는 연구 overlay다. 순수 계산·백테스트 전용이며 paper runner,
  텔레그램, 주문, 운영 DB에 연결하지 않았다. 기존 전략 동작도 변경하지 않는다.
- 결론은 새 알파로 **NO-GO**다. 2026-01-02~07-24의 50종목 고정 소급 탐색은
  72거래·41활성일, NAV +3.21%, MDD -2.84%였다. 피크노출×일봉 proxy와의 단순
  격차는 -37.64%p지만 이 proxy는 장중 보유시간을 반영하지 않아 동일노출 벤치나
  초과수익이 아니다. 미래 선정 종목을 과거에 고정한 선택·생존편향까지 있으므로
  전략 성과로 해석하지 않는다.
- 섹터 `pick_date`와 7일 만료를 적용한 2026-06-28~07-10 근사 재현은 50종목의
  실데이터를 읽었지만 `v2_qv` 거래 0건이었다. 성공·실패 판정이 아니라 분봉 표본
  부족에 따른 판정 불가다. 레거시 스냅샷은 종목별 편입 시각을 보존하지 않으므로
  엄밀한 point-in-time 검증도 아니다.
- 월별 재분석은 1월부터 계속 하락장이 아니었다는 오너 지적을 반영했다. 기존 v2
  계열은 약세인 3·6월을 어느 정도 방어했으나 강한 1·2·4·5월 상승을 거의 포착하지
  못했다. `gm_v3`는 공유현금 NAV가 아니라 거래당 평균이라 직접 비교하지 않는다.
- 다음 연구 후보는 공식 지수·breadth가 확정된 risk-on에서 업종·종목 추세를
  보유하는 `gm_trend_v1`이다. neutral 신규진입 중단, 기존 포지션 트레일링,
  충분한 워밍업, point-in-time 유니버스, 공유현금, 동일노출 벤치를 처음부터
  강제하는 방향이지만 아직 구현 지시나 paper 승격 승인이 아니다.
- 2026-08-02 실측: `pytest tests/test_gm_regime_v1.py -q` → **16 passed**,
  `pytest tests/ -q` → **463 passed**, 기존 warning 1건, `git diff --check` 정상.
  두 백테스트 실행값도 `docs/research/gm_regime_v1.md`와 일치했다.
- 독립 리뷰가 최초 미추적본에서 ① 스냅샷의 종목별 편입시각 결손 ② 재편입 벤치의
  비활성 구간 수익 합산 ③ 최대노출×일봉을 동일노출 초과수익으로 부른 문제
  ④ 워밍업 결손의 risk-off 오분류를 지적했다. 재편입 계산과 `unknown` 분리를
  코드·테스트로 고쳤고, 스냅샷과 벤치는 각각 pick-date approximation 및
  피크노출 일봉 proxy로 명칭·해석을 격하했다. 종목별 이벤트 기반 엄밀한
  point-in-time/시간가중 동일노출 벤치는 다음 검증 과제로 남겼다.
...
## 2026-08-07 총기간 실제계좌형 수익 우선 표시

- 오너 지시에 따라 `paper_runner --report`와 텔레그램 마감 요약에서 기존 `v2`
  직렬복리를 봇의 총수익처럼 보이지 않게 분리했다.
- 대표 봇 수익은 공유현금·동시 5종목·종목당 20%·섹터당 2종목 제한을 반영한
  `v2_portfolio` NAV 하나로 정의한다. `v2_leader_portfolio`와
  `v2_qv_portfolio`는 대표값이 아닌 연구용 계좌형 관찰축이다.
- 총수익에는 실제 산출 시작일~최신일, 수익률, 시작자금 100의 NAV 변화,
  확정/장중 잠정 상태를 함께 표시한다. 기존 `v2`, GM, v4r 직렬복리는
  `연구용 참고지표 (실제 계좌 총수익 아님)`으로 명시한다.
- `bench_v2_portfolio`는 시작일만 맞춘 등록종목 100% 동일가중 보유이며 봇의
  현금/슬롯 노출과 매칭되지 않는다. 따라서 알파가 아니라 `동시작 시장 참고치
  (노출 비매칭)`와 `시장 참고차`로만 표시한다.
- 2026-08-06 실제 `paper.db` 확정 출력 예: 대표 총수익 `+0.28%`, NAV
  `100→100.28`, 동시작 시장 참고치 `+9.42%`. 실주문 손익은 아니다.
...
## 2026-08-08 운영 DB 전체 전략 진단

- 미니PC 전체 스냅샷 `20260807-225346`의 세 DB를 로컬 `verify`/`install`한 뒤
  v2 계열, GM 계열, v4r, 데이터 품질을 4개 독립 분석 작업으로 병렬 검증했다.
  운영 DB·서비스·주문 경로는 변경하지 않았다.
- 전략 승격은 **NO-GO**다. 2026-07-14 유니버스 75종목 중 72종목이 09:51에
  끝나 예상 분봉 약 83.0%가 없지만 확정 처리됐다. 유니버스 1,845행에는
  32 code-day 중복이 있고 v2 일별 선언 65건과 보존 원장 60건도 불일치한다.
- `trading.db`는 `pick_daily_tracking` 9,135행 중 3,003행 pending,
  8/4~7 일별 success 4/147(2.72%), `pick_minute_raw` 가격 0이
  16,687/32,923(50.7%)다. 시장·섹터 상대강도 컬럼은 전부 NULL이다.
- `paper_universe_log` 일별 유니버스와 Toss 분봉으로 종목당 20%, 동시 5종목,
  섹터당 2종목 계좌를 읽기 전용 재생성한 잠정값은 v2 -0.17%(MDD -4.87%),
  v2_qv -2.78%(MDD -3.91%)다. 불완전일 수정 전 최종 성과로 사용하지 않는다.
- v2 수익은 7/10 하루에 과도하게 집중됐고 10시 이후 합계 손실, 무TP EOD 27건
  평균 -0.77%였다. leader/qv는 승격 근거가 없다. GM 실청산 약 80%가 R10이며
  R13 추가 실청산 17건 중 14건이 R10, R14는 0회 발화다. v4r은 0TP/SL 61건이
  음의 수익 합계 약 99.3%를 만들고 최대 갭 손실은 -27.50%였다.
- 구현 순서는 세션 완결성 확정 게이트, 불완전일 재수집/전체 체인 재구축,
  stock code 중복 제거, trade_id/strategy_axis 분리와 원장 불변식, GM/v4r 공유현금
  계좌 및 동일노출 benchmark다. 전략 A/B는 이후 별도 축으로 동결한다.
- 상세 분석·수치·승격 기준은 `docs/research/db_strategy_diagnosis_20260808.md`에
  기록했다. 다음 구현은 데스크톱에서 최신 main 기준 별도 브랜치로 이어간다.
