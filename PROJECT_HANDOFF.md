...
> 마지막 업데이트: 2026-07-09 (웹 공유 접근 기능 — 아래 "최근 변경" 참고)
...
- **미결(오너 결정 대기)**: 기존 활성 섹터에 종목만 추가될 때는 upsert가 raw_input을 건드리지
  않아 추가자의 이름이 기록되지 않는다(픽 최초 등록자만 남음). 덮어쓰기/유지/종목단위 컬럼 중 결정 필요.
