...
## 9. 2026-08-08 운영 DB 전략 진단

- 전체 분석 결론과 수치는 `docs/research/db_strategy_diagnosis_20260808.md`를 따른다.
  전략 승격은 NO-GO이며 데이터 완결성·유니버스 중복·원장 불변식·공유현금 회계를
  전략 임계값보다 먼저 고친다.
- 잠정 point-in-time 공유현금 재생성은 v2 -0.17%, v2_qv -2.78%다. 7월 14일
  불완전 분봉이 남아 있어 최종 성과가 아니며 기존 v2 +24.09% 직렬복리와 혼동하지
  않는다.
- 다음 구현은 데스크톱에서 별도 브랜치로 진행한다. 노트북은 같은 변경을 병행하지
  않고 최신 main을 pull한 뒤 오너의 새 지시를 기다린다.
- 현재 기준본은 미니PC
