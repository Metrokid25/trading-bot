...
- `db/trading.db` — **노트북이 정식 누적 DB**. 분봉은 KIS 당일만 제공=백필 불가라 적립은 노트북 단일 소스.
...
- 웹 대시보드: `./.venv/Scripts/python.exe -m uvicorn webapp.server:app --host 127.0.0.1 --port 8000` (localhost 전용)
