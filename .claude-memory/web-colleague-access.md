...
- 2026-07-22 추가: 신형 ETF는 `0167A0`처럼 영문 혼합 코드가 있음. KIS 실측
  ...
  캐시 v3로 올린 `1fb64af` main push 완료, 미니PC는 아직 미배포.

## 2026-07-23 섹터명 대소문자 무시 (승인 대기)

- `ai솔루션`/`AI솔루션`을 같은 섹터로 취급하도록 저장소 등록·조회·삭제 키를
  공백 정리 + `casefold()`로 통일했다.
- uvicorn 시작 시 단순 case variant만 최신 표기로 통합한다. 이전 event/PDT/stock
  행은 원래 Pick에 보존하고 최신 활성 Pick에 누락 종목을 복제한다.
- 같은 표기의 정상 재픽이 여러 개이거나 다중 섹터 Pick과 섞인 모호한 데이터는
  자동 병합하지 않는다. 공백-only 섹터 입력은 422.
- 운영 배포 전 76종목·10섹터 → 현재 중복 기준 배포 후 75종목·9섹터 예상.
- 전체 407 passed, warning 1건. 독립 재리뷰 최종 승인. 커밋·push 전.
